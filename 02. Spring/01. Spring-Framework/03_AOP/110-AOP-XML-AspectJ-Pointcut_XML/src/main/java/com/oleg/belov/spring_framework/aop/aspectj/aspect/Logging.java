...
public class Logging {
    ...
    public void doBeforeGetAnything(JoinPoint joinPoint) {
        log.info("LOG POINTCUT 1: GET method called of " + joinPoint.getTarget().getClass().getName());
    }

    //@After("actionWithCity()")
    public void doAfterAnyActionCity(JoinPoint joinPoint) {
        log.info("LOG POINTCUT 2: CityManager method " + joinPoint.getSignature().getName() + " called, ends witt 'City'");
    }

    //@Before("allRegionManagerMethods()")
    public void doBeforeAnyActionRegion(JoinPoint joinPoint) {
        log.info("LOG POINTCUT 3: RegionManager method " + joinPoint.getSignature().getName() + " called");
    }

    //@After("allMethodInBean()")
    public void doAfterWithBean(JoinPoint joinPoint) {
		log.info("LOG POINTCUT 4: buildingManager method " + joinPoint.getSignature().getName() + " called");
    }
