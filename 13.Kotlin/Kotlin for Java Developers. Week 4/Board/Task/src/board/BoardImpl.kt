...
data class DefaultCell<T>(override val i: Int, override val j: Int, var value: T? = null) : Cell {
    override fun toString()= "($i, $j)"
}

open class DefaultSquareBoard<T>(final override val width: Int) : SquareBoard {
    protected val cells: List<DefaultCell<T>>

    init {
        if (width <= 0) throw IllegalArgumentException("Width must be a positive number, was: $width")
        cells = IntRange(1, width).flatMap { i -> IntRange(1, width).map { j -> DefaultCell<T>(i, j) } }
    }
...
class DefaultGameBoard<T>(width: Int) : DefaultSquareBoard<T>(width), GameBoard<T> {
    ...
    private fun cellMatcher(predicate: (T?) -> Boolean): (DefaultCell<T>) -> Boolean {
